...
class ProjectsScreenViewModel(
    ...
    private val _projectScreenState =
        MutableStateFlow<ProjectsScreenUiState>(ProjectsScreenUiState.Loading)
    val projectsScreenUiState = _projectScreenState.asStateFlow()
    ...
    private fun getProjects() {
        viewModelScope.launch {
            when (val result = projectRepository.getAllProjects(USER_ID)) {
                is Result.Error -> {
                    _projectScreenState.update { ProjectsScreenUiState.Error(result.message) }
                }

                is Result.Success -> {
                    _projectScreenState.update { ProjectsScreenUiState.Success(result.data) }
                }
            }
        }
    }
